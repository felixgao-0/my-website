...
function updateGraphs() {
  fetch("/data")
    ...
    .then((data) => {
      ...
      if (data === null) {
        // Print nothing when data missing for whatever reason
        ...
        console.debug("updated graphs without adding data");
        cpuGraph.update();
        memoryGraph.update();
        return;
      }
      ...
      // Save memory data
      pushShift(
        memoryDataset.global.globalUsage,
        convert(data.total.memory.used, "GB", 2),
      );

      // Memory Data
      pushShift(
        memoryGraph.data.datasets[0].data,
        convert(myMemUsage, "GB", 2),
      );
...
function selectButton(button, group) {
  const buttons = document.querySelectorAll(
    `.graph-selector[data-group="${group}"] .toggle-button`,
  ); // Get all toggle-btn classes in a group in a graph-selector class
  ...
  if (group == "cpu-options") {
    ...
    fetch("/data")
      ...
      .then((data) => {
        let chart_labels = [];
        let chart_data = [];
        let myStorageUsage = 0; // Initalize variable first

        data.by_dir.forEach((directory) => {
          if (
            directory[1] == "." ||
            directory[1] == "/home/runner/Nest-Website"
          ) {
            // Ignore the root filepaths
            return;
          } else if (directory[1] == "total") {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("/data")
    ...
    .then((data) => {
      ...
      cpuStats.addEventListener("mouseover", () => updateCpuTxt(data));
      cpuStats.addEventListener("mouseout", () => updateCpuTxt(data));
      ...
      memoryGraph = new Chart("memory-graph", {
        ...
        options: getOptionData(
          2,
          true,
          convert(data.total.memory.total, "GB", 2),
          " GB",
        ),
      });

      memoryStats.addEventListener("mouseover", () => updateMemoryTxt(data));
      memoryStats.addEventListener("mouseout", () => updateMemoryTxt(data));
      ...
      storageStats.addEventListener("mouseover", () => updateStorageTxt(data));
      storageStats.addEventListener("mouseout", () => updateStorageTxt(data));

      let myStorageUsage = null;
      data.by_dir.forEach((directory) => {
        // Add storage data :D
        if (directory[1] == ".") {
          // Ignore the root filepath
          return;
        }
        if (directory[1] == "total") {
          // Get total storage usage
          myStorageUsage = directory[0];
        }
      });
